...
public class AesEncryptionUtil {
    ...
    /**
     * Mask card hex for logging (show only last 4 characters)
     * @param cardHex Card hex string
     * @return Masked card hex
     */
    public static String maskCardHex(String cardHex) {
        if (cardHex == null || cardHex.length() < 4) {
            return "****";
        }
        int length = cardHex.length();
        return "****" + cardHex.substring(length - 4);
    }
}
